...
open class FGTask: FGObjectSerializable, CustomStringConvertible {
    ...
    /// When the task is create a list of filename is provided as input_files.
    /// These files have to be uploaded in order to make the task SCHEDULED.
    /// The task detail provide an object for the each input file with the attributes: name and status.
    /// The status can be NEEDED if the input file has not been provided or READY otherwise.
    public var inputFiles: [FGInputFile] = []

    /// Output files of the application.
    /// During the task creation the user can specify a list of files to retrieve after the execution.
    /// The task details provides an object for each output file containing the name and the URL for download.
    public var outputFiles: [FGOutputFile] = []

    /// Information of the running task provided back to the user.
    /// This is needed to allow users to interact with the application.
    /// As an example, for a task running a VM the runtime_data can contains the ip address and the credentials.
    /// The format is similar to parameters with the addition of two optional time fields: creation and last_change.
